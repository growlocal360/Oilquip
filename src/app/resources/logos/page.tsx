...
export default function LogosPage() {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      const response = await fetch("/api/resources?category=logos&published=true");
      if (response.ok) {
          ...
          >
            ...
            <div className="flex items-center space-x-4 mb-6">
              <div className="p-4 bg-accent-500/10 border border-accent-500/20 rounded-xl">
                <Image className="h-10 w-10 text-accent-500" />
              </div>
            </div>

            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-steel-100 mb-6">
              Oilquip <span className="text-gradient">Logos</span>
            </h1>
            <p className="text-xl text-steel-400 leading-relaxed max-w-2xl">
              Download official Oilquip logos in various formats and color
              variants for your marketing materials.
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {loading ? (
            ...
            <div className="text-center py-16">
              <Image className="h-16 w-16 text-steel-600 mx-auto mb-4" />
              <p className="text-steel-400 text-lg">
                No logos available yet. Check back soon!
              </p>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
