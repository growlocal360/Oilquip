...
export default function BrandGuidePage() {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      const response = await fetch("/api/resources?category=brand-guide&published=true");
      if (response.ok) {
          ...
          >
            ...
            <div className="flex items-center space-x-4 mb-6">
              <div className="p-4 bg-accent-500/10 border border-accent-500/20 rounded-xl">
                <BookOpen className="h-10 w-10 text-accent-500" />
              </div>
            </div>

            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-steel-100 mb-6">
              Brand <span className="text-gradient">Guide</span>
            </h1>
            <p className="text-xl text-steel-400 leading-relaxed max-w-2xl">
              Access our brand guidelines and style documentation to ensure
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-steel-400 text-lg">
                Brand guide coming soon. Check back later!
              </p>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
